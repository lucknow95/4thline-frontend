...
// Build-safe resolver that won't throw if the env var is missing/invalid.
function resolveMetadataBase(): URL {
  const fromNextPublic = process.env.NEXT_PUBLIC_BASE_URL;
  const fromVercel = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : null;

  // Prefer NEXT_PUBLIC_BASE_URL if it's a valid absolute URL.
  if (fromNextPublic) {
    try {
      return new URL(fromNextPublic);
    } catch {
      // fall through
    }
  }

  // Otherwise try VERCEL_URL (preview/prod deploys have this).
  if (fromVercel) {
    try {
      return new URL(fromVercel);
    } catch {
      // fall through
    }
  }
